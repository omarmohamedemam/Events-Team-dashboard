    ...
    <div className="grid-cols-1-2">
      <section className="card">
        ...
        <form action={saveWarning} className="card-body form-grid form-grid-1">
          ...
          <button className="btn btn-green" disabled={!hasMembers}>{hasMembers ? "Save warning" : "Add a member first"}</button>
        </form>
      </section>
      <section className="card">
        ...
        <div className="table-wrap"><table><thead><tr><th>Member</th><th>Type</th><th>Title</th><th>Status</th><th>Event</th></tr></thead><tbody>
          {warnings.length ? warnings.map((w) => <tr key={w.id}><td>{w.teamMember.fullName}</td><td>{w.noteType}</td><td>{w.title}</td><td>{w.status}</td><td>{w.event?.eventName || "-"}</td></tr>) : <tr><td colSpan={5}>No warnings yet.</td></tr>}
        </tbody></table></div>
      </section>
    </div>
  );
